 ...
 */
package com.freesia.homeownersnotices.core.functional
...
sealed class Either<out L, out R> {
     ...
     * @see Right
     */
    fun bind(fnL: (L) -> Any, fnR: (R) -> Any): Any =
        when (this) {
            is Left -> fnL(a)
            is Right -> fnR(b)
        }

    suspend fun doIfRight(fnR: suspend (R) -> Any): Any = when (this) {
        is Left -> Unit
        is Right -> fnR(b)
    }

    suspend fun doIfLeft(fnL: suspend (L) -> Any): Any = when (this) {
        is Left -> fnL(a)
        is Right -> Unit
    }
}
